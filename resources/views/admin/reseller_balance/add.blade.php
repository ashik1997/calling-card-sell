...
<div class="content-header">
    ...
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">Balance add</h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="#">Home</a></li>
          <li class="breadcrumb-item"><a href="#">Balance add</a></li>
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <div class="card card-default">
          ...
          <h3 class="card-title">Balance add</h3>
          <div class="card-tools">
            <a href="{{route('admin.reseller_balance.list')}}" class="btn btn-primary float-right"><i class="fa fa-arrow-left"></i> Back</a>
          </div>
      	</div>
      	<!-- /.card-header -->
      	@if ($errors->any())
        ...
        @endif
        <form action="{{ route('admin.reseller_balance.add') }}" method="post" enctype="multipart/form-data">
        	@csrf
	        <div class="card-body">
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  <div class="form-check-inline">
                    <label class="form-check-label">
                      <input type="radio" onchange="due_cal()" class="form-check-input" name="payment_type" value="balance" checked>Balance add
                    </label>
                  </div>
                  <div class="form-check-inline">
                    <label class="form-check-label">
                      <input type="radio" onchange="due_cal()" class="form-check-input" name="payment_type" value="return">Return
                    </label>
                  </div>
                  <div class="form-check-inline">
                    <label class="form-check-label">
                      <input type="radio" onchange="due_cal()" class="form-check-input" name="payment_type" value="payment">Payment
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div class="col-md-12" id="add_balance_hide_show">
                <div class="form-group">
                  <label for="add_balance">Add balance</label>
                  <input type="text" name="add_balance" id="add_balance" class="form-control" onkeyup="due_cal()" value="0" placeholder="Enter add balance">
                </div>
              </div>
...
<script>
  ...
  function due_cal(){
    ...
    let add_balance = $("#add_balance").val();
    if(add_balance==''){
      add_balance = 0;
    }
    ...
    $.ajax({
     ...
     success: function(data)
     {
      ...
      if (payment_type=='return') {
        $("#due").val(parseFloat(data[0])-parseFloat(add_balance));
        $("#paid_amount_hide_show").hide();
        $("#add_balance_hide_show").show();
        ...
        $("#due").val(parseFloat(data[0])+parseFloat(add_balance)-parseFloat(paid_amount));
        $("#paid_amount_hide_show").show();
        $("#add_balance_hide_show").show();
      }
     }
