      ...
      <div class="card">
        <div class="card-header bg-success">
          ...
          <div class="card-tools">
            <a href="{{route('reseller.card.sell_rate_plan')}}" class="btn btn-custom btn-sm"><i class="fa fa-arrow-left"></i> Back</a>
          </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
        ...
        <div class="card-footer">
          <div class="row">
            <div class="col-md-12">
              <a href="{{route('reseller.card_by_id',$card->id)}}?sell_rate_plan_id={{$_GET['sell_rate_plan_id']}}" class="btn btn-success btn-lg float-left" id="confirm"><i class="fa fa-check"></i> Confirm</a>
...
<script>
  ...
  $("#confirm").on('click',function(e){
    ...
    Swal.fire({
      ...
      $.ajax({
        url: "{{ route('reseller.password.check') }}",
        type: 'POST',
        dataType: "json",
        data: { password: result.value.password, _token: csrf },
          success: function(data){
            if (data==1) {
              window.location = "{{route('reseller.card_by_id',$card->id)}}?sell_rate_plan_id={{$_GET['sell_rate_plan_id']}}";
            }
            if (data==0) {
              Swal.fire({
                position: 'top-end',
